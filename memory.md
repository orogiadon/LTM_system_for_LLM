...
```
以下の記憶を要約してください。
- triggerは「何がきっかけだったか」を1文で要約
- contentは「どう反応したか」を1〜2文で要約
- 具体的な詳細は省略し、要点のみ残す
- 感情的なニュアンスがあれば残す
...
|------|-----|
| 1日の会話数 | 100件 |
| 1件あたりの文字数 | trigger 100文字 + content 400文字 = 500文字 |
| 運用期間 | 1年（365日） |
| 総記憶数 | 36,500件 |
...
| 用途 | トークン数 | コスト |
|------|-----------|--------|
| 新規記憶生成（trigger+content） | 27.4M | $0.55 |
| 想起時クエリ（ユーザー入力150文字×36,500回） | 8.2M | $0.16 |
| 圧縮時再生成（L1→L2） | 5.9M | $0.12 |
| 圧縮時再生成（L2→L3） | 1.2M | $0.02 |
| **合計** | **42.7M** | **$0.85** |

### LLM コスト（Claude 3.5 Haiku: $0.80/1M入力, $4.00/1M出力）

| 処理 | 入力トークン | 出力トークン | コスト |
|------|-------------|-------------|--------|
| 記憶生成（感情分析・要約） | 36.5M | 9.1M | $65.60 |
| L1→L2 圧縮（要約生成） | 27.1M | 5.9M | $45.28 |
| L2→L3 圧縮（キーワード抽出） | 7.5M | 1.2M | $10.80 |
| **合計** | **71.1M** | **16.2M** | **$121.68** |
...
| モデル | 年間コスト | 月額 |
|--------|-----------|------|
| **Claude 3.5 Haiku（推奨）** | **$122（¥18,300）** | **$10（¥1,500）** |
| Claude 4 Sonnet | $458（¥68,700） | $38（¥5,700）|
| Claude 4 Opus | $2,286（¥342,900） | $191（¥28,650） |

### 注意事項

- 上記は推定値であり、実際の使用量は会話パターンにより変動する
- 為替レートは 1 USD = 150 JPY で計算
- API価格は変更される可能性があるため、最新価格はAnthropic/OpenAI公式を確認
