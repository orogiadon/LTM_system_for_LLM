...
| レベル | 名称 | 保持スコア閾値 | 内容 |
|-------|------|---------------|------|
| 1 | 完全記憶 | > 50 | 会話の詳細、文脈、感情すべて保持 |
| 2 | 圧縮記憶 | 20 < x ≤ 50 | 要約のみ「○○について議論した」 |
| 3 | 痕跡記憶 | 5 < x ≤ 20 | キーワードと感情タグのみ |
...
for each memory:
  ...
  6. 自動関連付け（enable_auto_linking = true の場合）
       - 類似度が閾値以上の記憶ペアに same_topic 関連を追加

  7. アーカイブ自動削除（archive.auto_delete_enabled = true の場合）
...
保護記憶の一覧表示・解除を行うターミナルUI。`textual` または `rich` で実装する。
...
| 項目 | 説明 |
|------|------|
| 対象 | `protected = true` の記憶のみ |
| 一覧列 | チェックボックス / 日付 / trigger冒頭（30文字） / retention_score |
| プレビュー | カーソル位置の記憶の trigger全文 + content全文 |
| ソート | 日付順 / スコア順の切り替え |
| 操作 | チェックした記憶を一括unprotect |
...
記憶の分布は retention_score の自然減衰による閾値判定に基づいて決定される。各レベルの件数は減衰係数や想起頻度により動的に変化する。
